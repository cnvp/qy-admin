...
export function formatDate(time) {
  if (time) {
    const time_str = new Date(time)
    const y = time_str.getFullYear()
    const m = time_str.getMonth() + 1
    const d = time_str.getDate()
...
// 获取当前时间 hh:mm:ss
export function formatDateTime(time, noMinutes) {
  if (time) {
    time = Number(time)
    const time_str = new Date(time)
    if (!time_str) {
    ...
    const hh = time_str.getHours()
    const mm = time_str.getMinutes()
    const ss = time_str.getSeconds()
    if (noMinutes) {
