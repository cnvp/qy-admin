    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="170px">
      <el-form-item label="字段类型" prop="type">
        <el-select v-model="form.type" placeholder="字段类型" :disabled="isEdit" @change="changeType">
          ...
          <el-option value="varchar" label="字符" />
      ...
      <el-form-item v-if="!setup_filter(form.type)" label="字段相关设置">
        ...
        <el-row v-if="form.type === 'title' || form.type === 'varchar' || form.type === 'text' || form.type === 'number'" :gutter="20" type="flex" justify="start">
        ...
        <el-row v-if="form.type === 'typeid'" :gutter="20" type="flex" justify="start">
          ...
          <el-col :span="3"><el-input v-model="form.setup['typeid']" /></el-col>
        </el-row>
        ...
        <el-row v-if="form.type === 'text' || form.type === 'varchar' || form.type === 'textarea' || form.type === 'editor' || form.type === 'select' || form.type === 'number'" :gutter="20" type="flex" justify="start">
      ...
      <el-form-item v-if="form.type === 'datetime'" label="时间字段类型设置" required>
        <el-radio-group v-model="form.setup['currenttime']">
          <el-radio :label="1">当前时间(timestamp)</el-radio>
          <el-radio :label="0">设置时间(varchar)</el-radio>
        </el-radio-group>
      </el-form-item>
  ...
  methods: {
    ...
    changeSource(id) {
      this.source_arr.map(item => {
        if (item.id === id) {
          this.$set(this.form.setup, 'source_moduleid', item.moduleid)
