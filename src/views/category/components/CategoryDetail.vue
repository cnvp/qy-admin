<template>
  <div class="createPost-container">
    <el-form ref="postForm" :model="postForm" :rules="rules" class="form-container" label-width="120px">
      ...
      <el-form-item label="栏目类型:" class="postInfo-container-item" prop="type">
        <el-radio-group v-model="postForm.type" :disabled="isEdit" @change="changeType">
          <el-radio :label="1">外链</el-radio>
          <el-radio :label="0">多栏目结构</el-radio>
          <el-radio :label="2">树形结构</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item v-if="!postForm.ischild && postForm.type !== 1" label="请选择模型" class="postInfo-container-item" prop="moduleid">
        <el-select v-model="postForm.moduleid" :disabled="isEdit" @change="changeCateogryModule">
          <template v-for="item in module_arr">
            <el-option v-if="item.type == postForm.type" :key="item.id" :value="item.id" :label="item.title || item.names" />
          </template>
        </el-select>
      </el-form-item>
      ...
      <el-form-item v-if="postForm.type === 1" label="外链地址:" class="postInfo-container-item" prop="url">
        <el-input v-model="postForm.url" />
      </el-form-item>
      <el-form-item v-if="postForm.type !== 1" label="栏目目录:" class="postInfo-container-item" prop="path">
        <el-input v-model="postForm.path" />
      </el-form-item>
      <el-form-item v-if="field_arr.length > 0 && postForm.type !== 1" label="列表显示字段:" class="postInfo-container-item">
      ...
      <el-form-item v-if="field_arr.length > 0 && postForm.type !== 1" label="查询字段:" class="postInfo-container-item">
      ...
      <el-form-item label="是否显示导航:" class="postInfo-container-item" prop="ismenu">
        <el-radio-group v-model="postForm.ismenu">
          <el-radio :label="1">显示</el-radio>
          <el-radio :label="0">不显示</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="栏目图标:" class="postInfo-container-item">
        <el-input v-model="postForm.icon" style="width: 120px; margin-right: 20px;" /><svg-icon v-if="postForm.icon" :icon-class="postForm.icon" class-name="disabled" />
      </el-form-item>
      <el-form-item label="栏目分页数:" class="postInfo-container-item" prop="pagesize">
        <el-input v-model.number="postForm.pagesize" min="10" max="100" type="number" style="width: 120px;" />
      </el-form-item>
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        parentid: { required: true, message: '请输入上级栏目', trigger: 'change' },
        moduleid: { required: true, message: '请选择模型', trigger: 'change' },
  ...
  methods: {
    ...
    fetchData(id) {
      fetchCategoryDetail({ id }).then(data => {
        ...
        if (this.isEdit && !data.ischild && data.moduleid) {
    ...
    fetchModuleFieldList() {
      fetchModuleFieldList({ moduleid: this.postForm.moduleid, is_field: 1 }).then(data => {
        this.field_arr = data
      })
    },
    // 修改模型回调事件
    changeCateogryModule(value) {
      ...
      const modules = this.module_arr.filter(item => {
        return item.id === value
      })
      this.$set(this.postForm, 'type', Number(modules[0].type))
      this.fetchModuleFieldList()
    },
    // 修改栏目类型
    changeType(type) {
      this.$refs['postForm'].clearValidate()
      this.$set(this.postForm, 'moduleid', '')
    },
